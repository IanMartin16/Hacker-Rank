...
class Result {
    ...
    public static List<Integer> serviceLane(int n, List<List<Integer>> cases, List<Integer> width) {
        List<Integer> maxVehicleSizes = new ArrayList<>();
        
        for(List<Integer> caseItem : cases) {
            int start = caseItem.get(0);
            int end = caseItem.get(1);
            int maxSize = Integer.MAX_VALUE;
            
            for(int i = start; i <= end; i++) {
                maxSize = Math.min(maxSize, width.get(i));
            }
            maxVehicleSizes.add(maxSize);
        }
        return maxVehicleSizes;
